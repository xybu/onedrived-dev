...
class OneDriveLocalRepository:
    ...
    def __init__(self, context, drive, drive_config):
        """
    ...
    def _init_path_filter(self, ignore_file):
        try:
            ...
            self.logger.error('Failed to load ignore list file "%s": %s' % (ignore_file, e))
            rules = set()
    ...
    def _init_item_store(self):
        ...
        self._cursor.execute(_get_resource('items_db.sql', pkg_name='onedrived'))
        self._conn.commit()
